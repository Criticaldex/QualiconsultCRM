...
<div class="col-xs-12 col-sm-4 label" data-label="LBL_TIPOC">

{minify}
{capture name="label" assign="label"}{sugar_translate label='LBL_TIPOC' module='Calls'}{/capture}
{$label|strip_semicolon}:

{/minify}
</div>

<div class="col-xs-12 col-sm-8 edit-view-field " type="enum" field="tipoc_c"  >
{counter name="panelFieldCount" print=false}

{if !isset($config.enable_autocomplete) || $config.enable_autocomplete==false}
<select name="{$fields.tipoc_c.name}" 
id="{$fields.tipoc_c.name}" 
title=''       
>
{if isset($fields.tipoc_c.value) && $fields.tipoc_c.value != ''}
{html_options options=$fields.tipoc_c.options selected=$fields.tipoc_c.value}
{else}
{html_options options=$fields.tipoc_c.options selected=$fields.tipoc_c.default}
{/if}
</select>
{else}
{assign var="field_options" value=$fields.tipoc_c.options }
{capture name="field_val"}{$fields.tipoc_c.value}{/capture}
{assign var="field_val" value=$smarty.capture.field_val}
{capture name="ac_key"}{$fields.tipoc_c.name}{/capture}
{assign var="ac_key" value=$smarty.capture.ac_key}
<select style='display:none' name="{$fields.tipoc_c.name}" 
id="{$fields.tipoc_c.name}" 
title=''          
>
{if isset($fields.tipoc_c.value) && $fields.tipoc_c.value != ''}
...
<input
id="{$fields.tipoc_c.name}-input"
name="{$fields.tipoc_c.name}-input"
size="30"
value="{$field_val|lookup:$field_options}"
type="text" style="vertical-align: top;">
<span class="id-ff multiple">
<button type="button"><img src="{sugar_getimagepath file="id-ff-down.png"}" id="{$fields.tipoc_c.name}-image"></button><button type="button"
id="btn-clear-{$fields.tipoc_c.name}-input"
title="Clear"
onclick="SUGAR.clearRelateField(this.form, '{$fields.tipoc_c.name}-input', '{$fields.tipoc_c.name}');sync_{$fields.tipoc_c.name}()"><span class="suitepicon suitepicon-action-clear"></span></button>
</span>
{literal}
<script>
	...
	{/literal}
			...
			{literal}
		(function (){
			var selectElem = document.getElementById("{/literal}{$fields.tipoc_c.name}{literal}");
	...
	SUGAR.AutoComplete.{$ac_key}.inputNode = Y.one('#{$fields.tipoc_c.name}-input');
	SUGAR.AutoComplete.{$ac_key}.inputImage = Y.one('#{$fields.tipoc_c.name}-image');
	SUGAR.AutoComplete.{$ac_key}.inputHidden = Y.one('#{$fields.tipoc_c.name}');
			...
			//global variable 
			sync_{/literal}{$fields.tipoc_c.name}{literal} = function(){
				SyncToHidden();
			}
			function syncFromHiddenToWidget(){
				...
				for (i=0;i<selectElem.options.length;i++){
					...
					if (selectElem.options[i].value==selectElem.value && document.activeElement != document.getElementById('{/literal}{$fields.tipoc_c.name}-input{literal}'))
						SUGAR.AutoComplete.{/literal}{$ac_key}{literal}.inputNode.set('value',selectElem.options[i].innerHTML);
				}
			}

            YAHOO.util.Event.onAvailable("{/literal}{$fields.tipoc_c.name}{literal}", syncFromHiddenToWidget);
		{/literal}
...
<div class="col-xs-12 col-sm-4 label" data-label="LBL_COLABORADOR">

{minify}
{capture name="label" assign="label"}{sugar_translate label='LBL_COLABORADOR' module='Calls'}{/capture}
{$label|strip_semicolon}:

{/minify}
</div>

<div class="col-xs-12 col-sm-8 edit-view-field " type="relate" field="colaborador_c"  >
{counter name="panelFieldCount" print=false}

<input type="text" name="{$fields.colaborador_c.name}" class="sqsEnabled" tabindex="0" id="{$fields.colaborador_c.name}" size="" value="{$fields.colaborador_c.value}" title='' autocomplete="off"  	 >
<input type="hidden" name="{$fields.colaborador_c.id_name}" 
id="{$fields.colaborador_c.id_name}" 
value="{$fields.user_id_c.value}">
<span class="id-ff multiple">
<button type="button" name="btn_{$fields.colaborador_c.name}" id="btn_{$fields.colaborador_c.name}" tabindex="0" title="{sugar_translate label="LBL_ACCESSKEY_SELECT_USERS_TITLE"}" class="button firstChild" value="{sugar_translate label="LBL_ACCESSKEY_SELECT_USERS_LABEL"}"
onclick='open_popup(
"{$fields.colaborador_c.module}", 
600, 
400, 
"", 
true, 
false, 
{literal}{"call_back_function":"set_return","form_name":"EditView","field_to_name_array":{"id":"user_id_c","name":"colaborador_c"}}{/literal}, 
"single", 
true
);' ><span class="suitepicon suitepicon-action-select"></span></button><button type="button" name="btn_clr_{$fields.colaborador_c.name}" id="btn_clr_{$fields.colaborador_c.name}" tabindex="0" title="{sugar_translate label="LBL_ACCESSKEY_CLEAR_USERS_TITLE"}"  class="button lastChild"
onclick="SUGAR.clearRelateField(this.form, '{$fields.colaborador_c.name}', '{$fields.colaborador_c.id_name}');"  value="{sugar_translate label="LBL_ACCESSKEY_CLEAR_USERS_LABEL"}" ><span class="suitepicon suitepicon-action-clear"></span></button>
</span>
<script type="text/javascript">
SUGAR.util.doWhen(
		"typeof(sqs_objects) != 'undefined' && typeof(sqs_objects['{$form_name}_{$fields.colaborador_c.name}']) != 'undefined'",
		enableQS
);
</script>
</div>
...
<div class="col-xs-12 col-sm-4 label" data-label="LBL_CONTACTS_CALLS_1_FROM_CONTACTS_TITLE">

{minify}
{capture name="label" assign="label"}{sugar_translate label='LBL_CONTACTS_CALLS_1_FROM_CONTACTS_TITLE' module='Calls'}{/capture}
{$label|strip_semicolon}:

{/minify}
</div>

<div class="col-xs-12 col-sm-8 edit-view-field " type="relate" field="contacts_calls_1_name"  >
{counter name="panelFieldCount" print=false}

<input type="text" name="{$fields.contacts_calls_1_name.name}" class="sqsEnabled" tabindex="0" id="{$fields.contacts_calls_1_name.name}" size="" value="{$fields.contacts_calls_1_name.value}" title='' autocomplete="off"  	 >
<input type="hidden" name="{$fields.contacts_calls_1_name.id_name}" 
id="{$fields.contacts_calls_1_name.id_name}" 
value="{$fields.contacts_calls_1contacts_ida.value}">
<span class="id-ff multiple">
<button type="button" name="btn_{$fields.contacts_calls_1_name.name}" id="btn_{$fields.contacts_calls_1_name.name}" tabindex="0" title="{sugar_translate label="LBL_ACCESSKEY_SELECT_CONTACTS_TITLE"}" class="button firstChild" value="{sugar_translate label="LBL_ACCESSKEY_SELECT_CONTACTS_LABEL"}"
onclick='open_popup(
"{$fields.contacts_calls_1_name.module}", 
600, 
400, 
"", 
true, 
false, 
{literal}{"call_back_function":"set_return","form_name":"EditView","field_to_name_array":{"id":"contacts_calls_1contacts_ida","name":"contacts_calls_1_name"}}{/literal}, 
"single", 
true
);' ><span class="suitepicon suitepicon-action-select"></span></button><button type="button" name="btn_clr_{$fields.contacts_calls_1_name.name}" id="btn_clr_{$fields.contacts_calls_1_name.name}" tabindex="0" title="{sugar_translate label="LBL_ACCESSKEY_CLEAR_CONTACTS_TITLE"}"  class="button lastChild"
onclick="SUGAR.clearRelateField(this.form, '{$fields.contacts_calls_1_name.name}', '{$fields.contacts_calls_1_name.id_name}');"  value="{sugar_translate label="LBL_ACCESSKEY_CLEAR_CONTACTS_LABEL"}" ><span class="suitepicon suitepicon-action-clear"></span></button>
</span>
<script type="text/javascript">
SUGAR.util.doWhen(
		"typeof(sqs_objects) != 'undefined' && typeof(sqs_objects['{$form_name}_{$fields.contacts_calls_1_name.name}']) != 'undefined'",
		enableQS
);
</script>
</div>
...
<div class="col-xs-12 col-sm-4 label" data-label="LBL_OF_OFERTA_CALLS_1_FROM_OF_OFERTA_TITLE">

{minify}
{capture name="label" assign="label"}{sugar_translate label='LBL_OF_OFERTA_CALLS_1_FROM_OF_OFERTA_TITLE' module='Calls'}{/capture}
{$label|strip_semicolon}:

{/minify}
</div>

<div class="col-xs-12 col-sm-8 edit-view-field " type="relate" field="of_oferta_calls_1_name"  >
{counter name="panelFieldCount" print=false}

<input type="text" name="{$fields.of_oferta_calls_1_name.name}" class="sqsEnabled" tabindex="0" id="{$fields.of_oferta_calls_1_name.name}" size="" value="{$fields.of_oferta_calls_1_name.value}" title='' autocomplete="off"  	 >
<input type="hidden" name="{$fields.of_oferta_calls_1_name.id_name}" 
id="{$fields.of_oferta_calls_1_name.id_name}" 
value="{$fields.of_oferta_calls_1of_oferta_ida.value}">
<span class="id-ff multiple">
<button type="button" name="btn_{$fields.of_oferta_calls_1_name.name}" id="btn_{$fields.of_oferta_calls_1_name.name}" tabindex="0" title="{sugar_translate label="LBL_SELECT_BUTTON_TITLE"}" class="button firstChild" value="{sugar_translate label="LBL_SELECT_BUTTON_LABEL"}"
onclick='open_popup(
"{$fields.of_oferta_calls_1_name.module}", 
600, 
400, 
"", 
true, 
false, 
{literal}{"call_back_function":"set_return","form_name":"EditView","field_to_name_array":{"id":"of_oferta_calls_1of_oferta_ida","name":"of_oferta_calls_1_name"}}{/literal}, 
"single", 
true
);' ><span class="suitepicon suitepicon-action-select"></span></button><button type="button" name="btn_clr_{$fields.of_oferta_calls_1_name.name}" id="btn_clr_{$fields.of_oferta_calls_1_name.name}" tabindex="0" title="{sugar_translate label="LBL_ACCESSKEY_CLEAR_RELATE_TITLE"}"  class="button lastChild"
onclick="SUGAR.clearRelateField(this.form, '{$fields.of_oferta_calls_1_name.name}', '{$fields.of_oferta_calls_1_name.id_name}');"  value="{sugar_translate label="LBL_ACCESSKEY_CLEAR_RELATE_LABEL"}" ><span class="suitepicon suitepicon-action-clear"></span></button>
</span>
<script type="text/javascript">
SUGAR.util.doWhen(
		"typeof(sqs_objects) != 'undefined' && typeof(sqs_objects['{$form_name}_{$fields.of_oferta_calls_1_name.name}']) != 'undefined'",
		enableQS
);
</script>
</div>
...
addToValidate('EditView', 'user_id_c', 'id', false,'{/literal}{sugar_translate label='LBL_COLABORADOR_USER_ID' module='Calls' for_js=true}{literal}' );
addToValidate('EditView', 'of_oferta_calls_1_name', 'relate', false,'{/literal}{sugar_translate label='LBL_OF_OFERTA_CALLS_1_FROM_OF_OFERTA_TITLE' module='Calls' for_js=true}{literal}' );
addToValidate('EditView', 'accounts_calls_1_name', 'relate', false,'{/literal}{sugar_translate label='LBL_ACCOUNTS_CALLS_1_FROM_ACCOUNTS_TITLE' module='Calls' for_js=true}{literal}' );
addToValidate('EditView', 'colaborador_c', 'relate', false,'{/literal}{sugar_translate label='LBL_COLABORADOR' module='Calls' for_js=true}{literal}' );
addToValidate('EditView', 'contacts_calls_1_name', 'relate', false,'{/literal}{sugar_translate label='LBL_CONTACTS_CALLS_1_FROM_CONTACTS_TITLE' module='Calls' for_js=true}{literal}' );
addToValidate('EditView', 'tipoc_c', 'enum', false,'{/literal}{sugar_translate label='LBL_TIPOC' module='Calls' for_js=true}{literal}' );
...
addToValidateBinaryDependency('EditView', 'of_oferta_calls_1_name', 'alpha', false,'{/literal}{sugar_translate label='ERR_SQS_NO_MATCH_FIELD' module='Calls' for_js=true}{literal}: {/literal}{sugar_translate label='LBL_OF_OFERTA_CALLS_1_FROM_OF_OFERTA_TITLE' module='Calls' for_js=true}{literal}', 'of_oferta_calls_1of_oferta_ida' );
addToValidateBinaryDependency('EditView', 'colaborador_c', 'alpha', false,'{/literal}{sugar_translate label='ERR_SQS_NO_MATCH_FIELD' module='Calls' for_js=true}{literal}: {/literal}{sugar_translate label='LBL_COLABORADOR' module='Calls' for_js=true}{literal}', 'user_id_c' );
addToValidateBinaryDependency('EditView', 'contacts_calls_1_name', 'alpha', false,'{/literal}{sugar_translate label='ERR_SQS_NO_MATCH_FIELD' module='Calls' for_js=true}{literal}: {/literal}{sugar_translate label='LBL_CONTACTS_CALLS_1_FROM_CONTACTS_TITLE' module='Calls' for_js=true}{literal}', 'contacts_calls_1contacts_ida' );
</script><script language="javascript">if(typeof sqs_objects == 'undefined'){var sqs_objects = new Array;}sqs_objects['EditView_colaborador_c']={"form":"EditView","method":"query","modules":["Users"],"group":"or","field_list":["name","id"],"populate_list":["colaborador_c","user_id_c"],"required_list":["parent_id"],"conditions":[{"name":"name","op":"like_custom","end":"%","value":""}],"order":"name","limit":"30","no_match_text":"Sin coincidencias"};sqs_objects['EditView_assigned_user_name']={"form":"EditView","method":"get_user_array","field_list":["user_name","id"],"populate_list":["assigned_user_name","assigned_user_id"],"required_list":["assigned_user_id"],"conditions":[{"name":"user_name","op":"like_custom","end":"%","value":""}],"limit":"30","no_match_text":"Sin coincidencias"};sqs_objects['EditView_contacts_calls_1_name']={"form":"EditView","method":"get_contact_array","modules":["Contacts"],"field_list":["salutation","first_name","last_name","id"],"populate_list":["contacts_calls_1_name","contacts_calls_1contacts_ida","contacts_calls_1contacts_ida","contacts_calls_1contacts_ida"],"required_list":["contacts_calls_1contacts_ida"],"group":"or","conditions":[{"name":"first_name","op":"like_custom","end":"%","value":""},{"name":"last_name","op":"like_custom","end":"%","value":""}],"order":"last_name","limit":"30","no_match_text":"Sin coincidencias"};sqs_objects['EditView_of_oferta_calls_1_name']={"form":"EditView","method":"query","modules":["OF_oferta"],"group":"or","field_list":["name","id"],"populate_list":["of_oferta_calls_1_name","of_oferta_calls_1of_oferta_ida"],"required_list":["parent_id"],"conditions":[{"name":"name","op":"like_custom","end":"%","value":""}],"order":"name","limit":"30","no_match_text":"Sin coincidencias"};</script>{/literal}
